class Solution {
    ...
    private int helper(int sourceIdx, int targetIdx) {
        ...
        if (charsMatch) {
            ...
            var insertResult = helper(sourceIdx, targetIdx + 1);
            // delete a char
            var deleteResult = helper(sourceIdx + 1, targetIdx);
            // replace a char
            var replaceResult = helper(sourceIdx + 1, targetIdx + 1);
            result = 1 + Math.min(insertResult, Math.min(deleteResult, replaceResult));
        }
